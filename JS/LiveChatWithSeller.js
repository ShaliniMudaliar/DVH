...
async function getUsernameFromEmail() {
    const buyerEmail = localStorage.getItem("emailOrUsername"); // Stored in LocalStorage
...
async function getSellerUsernameFromUserID() {
    ...
    try {
        ...
        if (seller === buyerUsername) {
            console.warn("Buyer and seller are the same. Not setting currentChatUser.");
            return;
        }
    ...
    try {
        ...
        <div class="userRow">
            ...
            <div class="userInfo">
                ...
                ${chat.unreadCount > 0 ? `<span class="unreadBadge">${chat.unreadCount}</span>` : ""}
            </div>
        </div>
    `;
...
if (currentChatUser) {
    selectChatUser(document.querySelector(`.chatUser[data-username="${currentChatUser}"]`), currentChatUser);
}
...
async function loadMessages() {
    ...
    if (!receiver) {
        messagesDiv.innerHTML = "<p class='no-chat'>Select a user to start chat.</p>";
        return;
    }

    try {
        const res = await fetch(`http://localhost:5005/messages/${username}/${receiver}`);
...
function appendMessage(sender, message, timestamp) {
    const messagesDiv = document.getElementById("chatList");
    const div = document.createElement("div");
    const messageDate = new Date(timestamp).toLocaleDateString([], { weekday: 'short', month: 'numeric', day: 'numeric' });
...
async function sendMessage() {
    ...
    const messageData = { sender: username, receiver, message, timestamp: new Date() };
...
function openChat(chatWith) {
    ...
    socket.emit("markAsRead", { sender: chatWith, receiver: localStorage.getItem("buyerUsername") });
}
...
function deleteChatUser(username) {
    ...
    if (!confirm(`Are you sure you want to delete the chat with ${username}?`)) return;
    ...
    socket.emit("deleteChatUser", { user: localStorage.getItem("buyerUsername"), chatWith: username });

    // Clear messages if this chat was open
    if (sessionStorage.getItem("currentChatUser") === username) {
        document.getElementById("chatList").innerHTML = "<p class='no-chat'>Select a user to start chat.</p>";
...
async function initializeChat() {
    ...
    document.getElementById("chatList").addEventListener("dblclick", function (event) {

     // Ensure the double-click happened on the senderChat or receiverChat
     if (event.target.classList.contains("senderChat") || event.target.classList.contains("receiverChat")) {
