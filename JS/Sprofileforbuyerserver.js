...
app.get("/api/sellerDetails", async (req, res) => {
  ...
  try {
    ...
    let sellerData = await mongoDB.collection("Seller").findOne({ userID: inputUserId });
    if (sellerData) {
    ...
    if (!email) {
      return res.status(400).json({ error: "Seller not found in MongoDB and no email provided for MySQL lookup." });
    }
    ...
    sellerData = await mongoDB.collection("Seller").findOne({ userID: correctUserId });
    if (sellerData) {
      return res.json(sellerData);
    } else {
      return res.status(404).json({ error: "Seller details not found in MongoDB after MySQL lookup" });
    }
  } catch (error) {
...
app.get("/api/getActiveListings", async (req, res) => {
  ...
  try {
     ...
     const userIdInt = parseInt(userId, 10);
      ...
      const listings = await propertyCollection.find({ userId: userIdInt }).toArray();
...
app.get("/api/getPropertyStats", async (req, res) => {
  ...
  try {
      const totalListings = await mongoDB.collection("Property").countDocuments({ userId });
      const propertiesSold = await mongoDB.collection("Property").countDocuments({ userId, sellOrRent: "sell" });
      const propertiesRent = await mongoDB.collection("Property").countDocuments({ userId, sellOrRent: "rent" });
